...
│   ├── train_models.py
│   ├── evaluate.py
│   ├── figures.py               # generates reports/figures for the write-up
│   └── demo.py
├── results/
├── reports/
│   ├── labeling_rules.md
...
4. **Who touches what (high level)**  
   - **Ronald:** repo layout, `preprocess` / `clean_dataset` / `collect_yale_faqs`, Yale raw tables, `split_data`, `train_baseline`, `evaluate`, baseline `demo`.  
   - **Abel:** `src/train_models.py` (NB, calibrated Linear SVM, tuning) and companion metrics/joblibs under `results/`.  
   - **Kevin:** `src/demo` polish, `src/figures.py` outputs, Markdown → PDF workflow for `reports/final_report.md`.
...
`--model` to load any saved sklearn pipeline that exposes `predict_proba`:
...
Train Naive Bayes and calibrated Linear SVM comparators (`results/metrics_nb.json`, `results/metrics_svm.json`, joblibs under `results/`):
...
5. **Generate report figures and tables:**
   ...
   # writes into reports/figures/ :
   #   class_distribution.png, confusion_matrix_test.png,
   #   per_class_metrics.png, topk_accuracy.png,
   #   confidence_distribution.png, top_features_per_class.png,
   #   threshold_coverage.{png,csv}     (deployment trade-off curve)
   #   misclassified_examples.{csv,md}  (representative test errors)
   ```

   To re-render figures for a different trained pipeline:

   ```bash
   python -m src.figures --model results/multinomial_nb.joblib --out-dir reports/figures/nb
   ```

6. **Exploratory analysis:**

   `notebooks/exploration.ipynb` runs a full EDA on the processed corpus —
   class balance, length distributions, vocabulary stats, top discriminative
   tokens per class, cross-class Jaccard overlap, sample questions, and
   source-page diversity. Run after `clean_dataset` + `split_data`:
...
| **Abel** | Comparison models in `train_models.py` (NB, calibrated SVM) and `results/metrics_*.json` |
| **Kevin** | `demo` UX, `figures.py` report assets, PDF export for Canvas |
...
The write-up is in **`reports/final_report.md`** (aligned with
`reports/final_report_outline.md`: Introduction, Data, Methodology,
Implementation Details, Results, Error Analysis, Conclusion). Figures live in
`reports/figures/` and are produced by `python -m src.figures`. Section **5.6**
summarizes **Multinomial NB** and **calibrated Linear SVM** against the
logistic baseline using `results/metrics_nb.json` and `results/metrics_svm.json`.
The Canvas-ready PDF is **`reports/final_report.pdf`**; Appendix A in the
report documents how to rebuild it with `pandoc` and a headless browser.
...
| `reports/final_report.md` | Final report source (Markdown, tracked in git). |
| `reports/final_report.pdf` | Rendered PDF for Canvas submission (regenerated via Appendix A). |
| `reports/figures/*.png` | Auto-generated by `src/figures.py` (tracked in git so the report renders without re-running training). |
| `reports/figures/threshold_coverage.csv`, `misclassified_examples.{csv,md}` | Auxiliary tables produced by `src/figures.py`. |
| `notebooks/exploration.ipynb` | Full exploratory data analysis on the processed corpus. |
