...
5. **Generate report figures and tables:**
   ...
   To re-render figures for a different model (e.g. once Abel's are trained):

   ```bash
   python -m src.figures --model results/some_other_model.joblib --out-dir reports/figures/some_other
   ```
...
Working draft is in `reports/final_report.md` and follows the structure in
...
`reports/figures/` and are regenerated by `python -m src.figures`. The
*Comparison with NB / Linear SVM* sub-section in §5.6 is currently a stub
reserved for Abel's contribution. The rendered PDF is at
**`reports/final_report.pdf`** — see Appendix A in the report for the
two-line `pandoc` + Chrome-headless command that regenerates it.
...
| `reports/final_report.md` | Working draft of the final write-up (tracked in git). |
